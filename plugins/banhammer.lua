local function pre_process(msg)
  ...
  if msg.to.type == 'chat' then
    ...
    if banned or is_gbanned(user_id) then -- Check it with redis
      print('یوزر بن شده در حال صحبت است')
      local name = user_print_name(msg.from)
...
--vardump(extra)
      ...
      local receiver = "chat#id"..chat_id
       if get_cmd == "kick" then
         if member_id == from_id then
             return send_large_msg(receiver, "شما قادر به اخراج افراد نمی باشید")
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            return send_large_msg(receiver, "شما قادر به بن کردن افراد نمی باشید")
         end
      ...
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'یوزر @'..member..' ['..member_id..'] از بن گلوبالی باز شد')
...
local function run(msg, matches)
 if matches[1]:lower() == 'id' then
    ...
    elseif matches[1]:lower() == 'id' then
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "ای دی گروه برای" ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id  
    end
  end
  ...
  if matches[1]:lower() == 'unban' then -- /unban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	local name = user_print_name(msg.from)
        	savelog(msg.to.id, name.." ["..msg.from.id.."] بن یوزر باز شد ".. matches[2])
        	return 'یوزر '..user_id..' بن شد'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
...
if matches[1]:lower() == 'kick' then
	...
	if string.match(matches[2], '^%d+$') then
		...
		if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمی توانید مدیران و ناظمان را اخراج کنید"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "شما نمی توانید افراد را بن کنید"
		end
		...
		savelog(msg.to.id, name.." ["..msg.from.id.."] یوزر کیک شد ".. matches[2])
		kick_user(user_id, chat_id)
	else
		local cbres_extra = {
			chat_id = msg.to.id,
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return 'یوزر ['..user_id..' ] از لیست بن حذف شد'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unbanall',
