...
local function unlock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'no' then
    return 'اسم گروه در حال حاظر قابل تغییر است'
  else
    data[tostring(target)]['settings']['lock_name'] = 'no'
    save_data(_config.moderation.data, data)
  return 'اسم گروه قابل تغییر می باشد'
  end
end
...
local function lock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'حساسیت ضد اسپم قفل شد'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
  return 'حساسیت ضد اسپم باز شد'
  end
end
...
local function unlock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'حساسیت ضد اسپم قفل نمی باشد'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
  return 'حساسیت ضد اسپم باز می باشد'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'ورود اعضا در حال حاظر ممنوع است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'اعضا قفل شدند لفت دادن = بن شدن'
end
...
local function unlock_group_photomod(msg, data, target)
  ...
  if group_photo_lock == 'no' then
      return 'عکس گروه قابل تغییر است'
  else
...
local function set_description(target, about)
  ...
  save_data(_config.moderation.data, data)
  return 'درباره گروه تغییر یافت به to:\n'..about
end
  ...
  if msg.to.type ~= 'chat' then
    ...
    if matches[2] == 'ban' then
      ...
      if tonumber(matches[3]) == tonumber(msg.from.id) then 
        return "شما می توانید افراد را بن کنید"
      end
    ...
    if matches[2] == 'kick' then
      ...
      local user_id = matches[3]
      if tonumber(matches[3]) == tonumber(msg.from.id) then 
        return "شما می توانید افراد را اخراج کنید"
      end
    ...
    if matches[2] == "setflood" then
      ...
      if tonumber(matches[3]) < 5 or tonumber(matches[3]) > 20 then
        return "عدد اشتباه است باید بین 5 تا 20 باشد"
      end
      ...
      return 'حساسیت ضد اسپم تغییر کرد به '..matches[3]
    end
    ...
    if matches[2] == 'new' then
      if matches[3] == 'link' then
        ...
        return "لینک جدید ایجاد شد \n  شما می توانید با دستور آن را دریافت کنید"..matches[1].."دریافت لینک جدید"
      end
    end
    if matches[2] == 'get' then 
      if matches[3] == 'link' then
        ...
        if not group_link then 
          return "ابتدا با دستور""[/newlink]"لینک جدیدی بسازید"
        end
        local name = user_print_name(msg.from)
